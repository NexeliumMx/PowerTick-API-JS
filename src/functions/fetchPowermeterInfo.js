/**
 * FileName: src/functions/fetchPowermeterInfo.js
 * Author(s): Arturo Vargas Cuevas
 * Brief: HTTP GET endpoint to fetch properties of a specific powermeter.
 * Date: 2025-02-24
 *
 * Description:
 * This function serves as an HTTP GET endpoint to fetch properties of a specific powermeter.
 * It verifies that the user has access to the powermeter and then retrieves all properties of the powermeter.
 * The function obtains its query from the file:
 *    PowerTick-backend/postgresql/dataQueries/fetchData/fetchPowermeterInfo.sql
 * 
 ...
 *
 * 4. Query Execution: It executes a query to fetch all properties of the specified powermeter, 
...
app.http('fetchPowermeterInfo', {
    ...
    handler: async (request, context) => {
        ...
        try {
            ...
            // Query to fetch properties of the specified powermeter
            const query = `
                WITH user_access AS (
                    SELECT 
                        1
