/**
 * FileName: src/functions/demandHistory.js
 * Author(s): Arturo Vargas
 * Brief: HTTP GET endpoint to fetch power demand history data for a specific powermeter in the selected schema (production, dev, or demo).
 * Date: 2025-02-24
 *
 * Description:
 * This function serves as an HTTP GET endpoint to fetch power demand history data for a specific powermeter in the selected schema (production, dev, or demo).
 ...
 *
 * 4. Query Execution: It executes a query to fetch the demand history data for the specified powermeter and time interval, 
 ...
 * Local:
 *    curl -i -X GET "http://localhost:7071/api/demandHistory?user_id=4c7c56fe-99fc-4611-b57a-0d5683f9bc95&serial_number=DEMO000001&time_interval=day&schema=demo"
 *    curl -i -X GET "http://localhost:7071/api/demandHistory?user_id=4c7c56fe-99fc-4611-b57a-0d5683f9bc95&serial_number=DEMO000001&time_interval=month&schema=demo" 
 *    curl -i -X GET "http://localhost:7071/api/demandHistory?user_id=4c7c56fe-99fc-4611-b57a-0d5683f9bc95&serial_number=DEMO000001&time_interval=year&schema=demo"
 * Production (schema omitted, defaults to production):
 *    curl -i -X GET "https://power-tick-api-js.nexelium.mx/api/demandHistory?user_id=4c7c56fe-99fc-4611-b57a-0d5683f9bc95&serial_number=DEMO000001&time_interval=day"
 *    curl -i -X GET "https://power-tick-api-js.nexelium.mx/api/demandHistory?user_id=4c7c56fe-99fc-4611-b57a-0d5683f9bc95&serial_number=DEMO000001&time_interval=month"
 *    curl -i -X GET "https://power-tick-api-js.nexelium.mx/api/demandHistory?user_id=4c7c56fe-99fc-4611-b57a-0d5683f9bc95&serial_number=DEMO000001&time_interval=year"
 *
 * Expected Response:
 * [{"timestamp_utc":"2025-02-25T02:30:00.000Z","timestamp_tz":"2025-02-25T02:30:00.000Z","avg_real_power_w":123.45,"max_real_power_w":150.67,"avg_var":67.89,"max_var":80.12}, ...]
 * --------------------------------------------------------------------------- 
*/
    ...
    handler: async (request, context) => {
        ...
        if (timeInterval === 'day') {
            query = `
                ...
                SELECT "timestamp_utc", "timestamp_tz", AVG(watts) AS avg_real_power_w, MAX(watts) AS max_real_power_w, AVG(var) AS avg_var, MAX(var) AS max_var
                FROM ${schema}.measurements
                WHERE serial_number = $2
                    AND "timestamp_tz" < NOW() AT TIME ZONE (SELECT time_zone FROM powermeter_time_zone)
                    AND EXISTS (SELECT 1 FROM user_access)
                GROUP BY "timestamp_utc", "timestamp_tz"
            ...
            query = `
                ...
                SELECT date_trunc('day', "timestamp_utc") AS day, AVG(watts) AS avg_real_power_w, MAX(watts) AS max_real_power_w, AVG(var) AS avg_var, MAX(var) AS max_var
                FROM ${schema}.measurements
                WHERE serial_number = $2
                    AND "timestamp_tz" < NOW() AT TIME ZONE (SELECT time_zone FROM powermeter_time_zone)
                    AND EXISTS (SELECT 1 FROM user_access)
            ...
            query = `
                ...
                SELECT date_trunc('month', "timestamp_utc") AS month, AVG(watts) AS avg_real_power_w, MAX(watts) AS max_real_power_w, AVG(var) AS avg_var, MAX(var) AS max_var
                FROM ${schema}.measurements
                WHERE serial_number = $2
                    AND "timestamp_tz" < NOW() AT TIME ZONE (SELECT time_zone FROM powermeter_time_zone)
                    AND EXISTS (SELECT 1 FROM user_access)
