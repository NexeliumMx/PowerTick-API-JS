/**
 * File: consumptionProfile.js
 * Author(s): Arturo Vargas
 * Endpoint: /api/consumptionProfile
 * Brief: Returns grouped energy consumption profile (hourly, daily, or monthly).
 * Date: 2025-06-02
 * 
...
app.http('consumptionProfile', {
    ...
    handler: async (request, context) => {
        ...
        // --- Build dynamic SQL ---
        // CTE for powermeter info with time zone and offset
        const ctes = `
        ...
        if (time_interval === 'hour') {
            selectQuery = `
                SELECT
                    ...
                    last(m.kwh_imported_total, m."timestamp") - first(m.kwh_imported_total, m."timestamp") AS wh,
                    last(m.varh_imported_q1, m."timestamp") - first(m.varh_imported_q1, m."timestamp") AS varh
