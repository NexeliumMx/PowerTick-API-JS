/**
 * File: thdVoltageLNProfile.js
 * Author(s): Arturo Vargas
 * Endpoint: /api/thdVoltageLNProfile
 * Brief: Returns grouped THD voltage LN profile (hourly, daily, or monthly) for a powermeter, validating user access and UTC range.
 * Date: 2025-06-23
 * 
        ...
        if (time_interval === 'hour') {
            selectQuery = `
                SELECT
                    ...
                    max(m.thd_voltage_ln) AS thd_voltage_ln_max,
                    avg(m.thd_voltage_ln) AS thd_voltage_ln_avg,
                    max(m.thd_voltage_l1) AS thd_voltage_l1_max,
                    avg(m.thd_voltage_l1) AS thd_voltage_l1_avg,
                    max(m.thd_voltage_l2) AS thd_voltage_l2_max,
                    avg(m.thd_voltage_l2) AS thd_voltage_l2_avg,
                    max(m.thd_voltage_l3) AS thd_voltage_l3_max,
                    avg(m.thd_voltage_l3) AS thd_voltage_l3_avg
